// Test HERE.com city discovery using correct parameters
...
async function findCitiesNearby(lat, lon, radius) {
    try {
        // Use discover endpoint with correct parameters
        const response = await axios.get('https://discover.search.hereapi.com/v1/discover', {
            params: {
                ...
                limit: 100,
                lang: 'en-US',
                // Use proper query for populated places
                q: 'populated places',
                // Filter by country
                in: 'countryCode:USA',
                // Result filtering
                result_types: 'administrativeArea'
            }
        });
        ...
        const cities = response.data.items
            .filter(item => {
                return (
                    ...
                    item.administrativeArea?.type === 'city' &&
                    item.address &&
                    item.address.city &&
            ...
            .map(item => ({
                ...
                type: item.administrativeArea.type,
                position: item.position,
                population: item.administrativeArea.population
            }))
            // Remove duplicates
...
async function runTests() {
    console.log('🌎 Testing HERE.com City Discovery (Discover Method)');
