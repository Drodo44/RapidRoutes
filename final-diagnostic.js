...
Promise.all([
  ...
  const { validateLane, getEquipmentWeightLimit } = validationModule;
  const { generateDatCsvRows, toCsv } = csvModule;

  console.log('\n📊 DAT WEIGHT LIMITS VERIFICATION:');
  console.log('V (Dry Van):', getEquipmentWeightLimit('V'), 'lbs');
  console.log('R (Reefer):', getEquipmentWeightLimit('R'), 'lbs');
  console.log('F (Flatbed):', getEquipmentWeightLimit('F'), 'lbs');
