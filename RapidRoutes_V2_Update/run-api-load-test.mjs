#!/usr/bin/env node
/**
 * Production environment variables wrapper for load test script
 */
...
// Run load test script directly using spawnSync to capture output
import { spawnSync } from 'child_process';

console.log('🚀 Running RapidRoutes API load test script...');

const result = spawnSync('node', ['scripts/load-test-intelligence-api.mjs'], { 
  env: process.env,
  stdio: 'inherit',
  encoding: 'utf-8'
});
...
if (result.status !== 0) {
  console.error(`❌ Load test failed with code ${result.status}`);
