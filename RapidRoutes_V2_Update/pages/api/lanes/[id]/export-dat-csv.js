...
function buildDatRow(lane, originCity, destCity, contactMethod, rrNumber) {
  ...
  // Generate rate (randomize if toggled)
  let rate = '';
  if (lane.randomize_rate) {
      const min = Number(lane.rate_min);
      const max = Number(lane.rate_max);
      if (min && max) {
          rate = String(getRandomWeight(min, max)); // Reusing getRandomWeight as logic is identical
      }
  } else if (lane.rate) {
...
async function handler(req, res) {
  ...
  try {
    ...
    console.log('[TRACE] Generating Lane CSV with BLANK Ref ID');
