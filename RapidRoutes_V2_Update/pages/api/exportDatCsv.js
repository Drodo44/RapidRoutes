...
async function selectLanes({ pending, days, all }) {
  ...
  // (Previous implementation queried dat_loads_2025 which was incorrect)
  ...
  // limit to 2000 to prevent timeouts
  const { data: lanes, error } = await query.limit(2000);
  
  if (error) {
    console.error('Error fetching lanes for CSV:', error);
    throw error;
  }
  ...
  try {
    ...
    const generator = new EnterpriseCsvGenerator({
      generation: {
        ...
        enableCaching: false // DISABLED: Force fresh generation
      },
