...
// Updated: Added 19 South Jersey cities (NJ_ATL + PA_PHI KMAs) for better Millville coverage
...
export async function generateGeographicCrawlPairs(origin, dest, radiusMiles = 75) {
  ...
  const originState = normalizeState(origin.state || origin.state_or_province);
  
  let destCities;
  
  // Special handling for large states - return ALL cities statewide for FL, CA, TX
  const isLargeState = ['FL', 'CA', 'TX'].includes(destState) || ['FL', 'CA', 'TX'].includes(originState);
  const largeStateCode = ['FL', 'CA', 'TX'].find(s => s === destState || s === originState);
  
  if (isLargeState && largeStateCode) {
    const stateEmoji = largeStateCode === 'FL' ? '🌴' : largeStateCode === 'CA' ? '☀️' : '🤠';
    console.log(`${stateEmoji} ${largeStateCode} lane detected - loading ${largeStateCode} cities statewide`);
    const { data: allStateCities, error: stateError } = await supabaseAdmin
      .from('cities')
      .select('*')
      .eq('state_or_province', largeStateCode)
      .not('kma_code', 'is', null);
    
    if (stateError) {
      console.error(`Error loading ${largeStateCode} cities:`, stateError);
      destCities = await findNearbyKMAs(Number(dest.latitude), Number(dest.longitude), radiusMiles);
    } else {
      // Calculate distance for all cities
      const citiesWithDistance = allStateCities.map(city => ({
        ...city,
      ...
      // Group by KMA and take top 20 closest cities per KMA
      const byKMA = {};
      ...
      // Sort each KMA group by distance and take top 20
      destCities = [];
      Object.keys(byKMA).forEach(kma => {
        const sorted = byKMA[kma].sort((a, b) => a.distance - b.distance);
        destCities.push(...sorted.slice(0, 20)); // Top 20 per KMA
      });
      
      console.log(`✅ Selected ${destCities.length} ${largeStateCode} cities (top 20 per KMA from ${Object.keys(byKMA).length} markets)`);
    }
  } else {
    destCities = await findNearbyKMAs(
      Number(dest.latitude),
      Number(dest.longitude),
      radiusMiles
    );
  ...
  // For large state lanes (FL, CA, TX), keep only cities in that state
  if (isLargeState && largeStateCode) {
    destCities = destCities.filter(c => normalizeState(c.state || c.state_or_province) === largeStateCode);
    const stateEmoji = largeStateCode === 'FL' ? '🌴' : largeStateCode === 'CA' ? '☀️' : '🤠';
    console.log(`${stateEmoji} Filtered to ${destCities.length} ${largeStateCode}-only destination cities`);
  }
  ...
  for (const orig of originCities) {
    for (const dest of destCities) {
      ...
      // Use city+state combination for deduplication instead of just KMA codes
      // This allows multiple cities from the same KMA (e.g., Houston metro area)
      const pairKey = `${orig.city}_${orig.state || orig.state_or_province}-${dest.city}_${dest.state || dest.state_or_province}`;
  ...
  // Validate minimum KMA requirements - prefer 6 unique KMAs but allow flexibility
  // for large metro areas where one KMA may cover many cities (e.g., Houston, Dallas)
  if (seenPairs.size < MIN_UNIQUE_KMAS) {
    // Check if we have enough total pairs to proceed despite low KMA diversity
    const MIN_PAIRS_OVERRIDE = 20; // If we have 20+ pairs from the same KMA, allow it
    
    if (pairs.length >= MIN_PAIRS_OVERRIDE) {
      console.warn(`⚠️ GEOCRAWL WARNING: Low KMA diversity (${seenPairs.size}/${MIN_UNIQUE_KMAS}) but sufficient pairs (${pairs.length}). Proceeding.`, {
