...
export async function verifyCityWithHERE(city, state, zip = null, verificationType = 'automatic', verifiedBy = null) {
  if (!HERE_API_KEY) {
    console.warn('⚠️ HERE_API_KEY not configured - skipping verification');
    return {
      verified: true, // Assume verified when API not available
      skipReason: 'HERE_API_KEY not configured',
      responseTime: 0
    };
  }
  ...
  try {
    ...
    const url = `${HERE_GEOCODING_URL}?q=${encodeURIComponent(query)}&countryCode=USA&apikey=${HERE_API_KEY}&limit=100`;
 ...
 */
export async function generateAlternativeCitiesWithHERE(centerLat, centerLng, radiusMiles = 75, limit = 20) {
  if (!HERE_API_KEY) {
    console.error('❌ HERE_API_KEY not configured');
    throw new Error('HERE API key not configured');
  }

  // Cap limit at 100 to prevent HERE.com API limit exceeded errors
  const cappedLimit = Math.min(100, limit || 20);

  console.log(`🌎 Generating alternatives within ${radiusMiles} miles of ${centerLat}, ${centerLng} (limit: ${cappedLimit})`);

  try {
    // Convert miles to meters for HERE.com API (must be integer)
    const radiusMeters = Math.round(radiusMiles * 1609.34);

    // Use discover API with at parameter and q=city (using correct lowercase apikey parameter)
    const url = `https://discover.search.hereapi.com/v1/discover?at=${centerLat},${centerLng}&q=city&limit=${cappedLimit}&apikey=${HERE_API_KEY}`;
    ...
    const data = await response.json();
    console.log(`✅ HERE.com alternatives generated: ${data.items.length} results`);

    return data.items;
  } catch (error) {
    console.error('❌ Error generating alternative cities:', error);
    throw error;
  }
}
