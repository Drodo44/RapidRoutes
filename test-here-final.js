// Test HERE.com city discovery using autosuggest endpoint with proper types
...
async function findCitiesNearby(lat, lon, radius) {
    try {
        // Use autosuggest endpoint with proper place types
        const response = await axios.get('https://autosuggest.search.hereapi.com/v1/autosuggest', {
            params: {
                ...
                limit: 100,
                lang: 'en-US',
                // Query for populated places
                q: 'populated place',
                // Use supported place types
                types: 'place',
                // Geographic filters
                in: `countryCode:USA`,
                politicalView: 'USA'
        ...
        const cities = response.data.items
            .filter(item => {
                return (
                    ...
                    item.address.state &&
                    // Only include actual cities/places
