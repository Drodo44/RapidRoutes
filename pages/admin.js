...
function AdminPage() {
  ...
  const loadPromotionRequests = async () => {
    ...
    try {
      ...
      setPromotionRequests(result.requests || []);
    } catch (error) {
  ...
  const handlePromotionAction = async (requestId, action, reason = null) => {
    setProcessingRequest(prev => ({ ...prev, [requestId]: true }));
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const response = await fetch('/api/promotions/approve', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        await loadUsers();
        alert(`Promotion request ${action === 'approve' ? 'approved' : 'denied'} successfully!`);
      } else {
        const result = await response.json();
      ...
      alert('Failed to process promotion request');
    } finally {
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '16px' }}>
        ...
        {/* Promotion Requests */}
        <Section title="🚀 Broker Promotion Requests">
          <div style={{ marginBottom: '16px' }}>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '12px' }}>
              Apprentices and Support staff can request promotion to Broker status to create their own teams.
            </p>
            <button
              onClick={loadPromotionRequests}
              className="btn btn-sm"
              style={{
                background: 'var(--bg-tertiary)',
                color: 'var(--text-secondary)',
            ...
            >
              {promotionsLoading ? 'Refreshing…' : 'Refresh Requests'}
            </button>
          </div>
                ...
                .map(request => (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button
                          onClick={() => handlePromotionAction(request.id, 'approve')}
                        ...
                        <button
                          onClick={() => {
                            ...
                            handlePromotionAction(request.id, 'deny', reason);
                          }}
                          disabled={processingRequest[request.id]}
                          className="btn btn-sm"
                          style={{
          ...
          {/* Show processed requests in collapsed section */}
          {promotionRequests.filter(r => r.status !== 'pending').length > 0 && (
            <details style={{ marginTop: '20px' }}>
              <summary style={{ 
                ...
                View Processed Requests ({promotionRequests.filter(r => r.status !== 'pending').length})
              </summary>
                  ...
                  .map(request => (
                    <div
                      ...
                      style={{
                        padding: '12px',
                        borderRadius: 'var(--radius)',
                        border: `1px solid ${request.status === 'approved' ? 'var(--success)' : 'var(--danger)'}`,
                        backgroundColor: request.status === 'approved' ? 'var(--success-light)' : 'var(--danger-light)',
                        fontSize: '12px'
                      }}
                    >
                      <div style={{ fontWeight: 600, marginBottom: '4px' }}>
                        {request.user_email} - {request.status === 'approved' ? '✓ Approved' : '✗ Denied'}
                      </div>
                      <div style={{ color: 'var(--text-secondary)' }}>
                        Team: {request.requested_team_name} | Reviewed {new Date(request.reviewed_at).toLocaleDateString()}
                      </div>
                      {request.rejection_reason && (
                        <div style={{ color: 'var(--danger)', marginTop: '4px', fontStyle: 'italic' }}>
