...
async function testCityDiscovery() {
    ...
    for (const location of testLocations) {
        ...
        try {
            // Try the geocoding endpoint instead
            const response = await axios.get('https://geocode.search.hereapi.com/v1/geocode', {
                params: {
                    apiKey: HERE_API_KEY,
                    q: `cities within 75 miles of ${location.name}`,
                    limit: 100,
                    lang: 'en',
                    types: 'city'
                }
            ...
            if (response.data && response.data.items) {
                ...
                const cities = response.data.items.filter(item => {
                    ...
                    if (!item.address || !item.address.state || item.address.countryCode !== 'USA') {
                        return false;
                    }

                    // Must be a city/municipality
                    if (!item.resultType || item.resultType !== 'locality') {
                        return false;
                    }

                    // Exclude things that are clearly not cities
                    const title = item.title.toLowerCase();
                    const exclusions = ['street', 'road', 'avenue', 'boulevard', 'lane', 'drive', 'way', 'circle'];
                    if (exclusions.some(ex => title.includes(ex))) {
                        return false;
                    }
                ...
                cities.forEach(item => {
                    ...
                    if (distance <= 75) { // Verify within 75 miles
                        ...
                        console.log(`  County: ${item.address.county || 'N/A'}`);
