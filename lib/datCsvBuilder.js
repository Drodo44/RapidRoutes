...
export async function planPairsForLane(lane, { preferFillTo10 = false, usedCities = new Set() } = {}) {
  ...
  try {
    ...
    // EMERGENCY FIX: If no pairs generated, create basic posting
    if (crawl.pairs.length === 0) {
      console.warn(`⚠️ EMERGENCY BYPASS: No pairs generated, creating basic posting for lane ${safeLane.id}`);
      
      // Create at least 5 basic pairs using same origin/destination
      const emergencyPairs = [];
      for (let i = 0; i < 5; i++) {
        emergencyPairs.push({
          pickupCity: baseOrigin.city,
          pickupState: baseOrigin.state,
          deliveryCity: baseDest.city, 
          deliveryState: baseDest.state,
          intelligence_type: 'emergency_bypass'
        });
      }
...
export function rowsFromBaseAndPairs(lane, baseOrigin, baseDest, pairs, preferFillTo10 = false, usedRefIds = new Set()) {
  ...
  // EMERGENCY DEBUG: Log what we actually receive
  console.log(`🚨 ROWSFROMPAIRS Lane ${safeLane.id} RECEIVED:`, {
    baseOrigin: baseOrigin ? `${baseOrigin.city}, ${baseOrigin.state}` : 'null',
    baseDest: baseDest ? `${baseDest.city}, ${baseDest.state}` : 'null', 
    pairsReceived: pairs ? pairs.length : 'null/undefined',
    pairsType: typeof pairs,
    preferFillTo10
  });
