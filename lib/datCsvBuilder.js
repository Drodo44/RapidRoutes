...
function baseRowFrom(lane, origin, dest, contact, usedRefIds = new Set()) {
  ...
  return {
    ...
    'Origin City*': origin.zip ? '' : origin.city, // BLANK if ZIP available, else city name
    'Origin State*': origin.zip ? '' : origin.state, // BLANK if ZIP available, else state abbreviation
    'Origin Postal Code': origin.zip ? `="` + (origin.zip || '') + `"` : '', // TEXT FORMAT for leading zeros
    'Destination City*': dest.zip ? '' : dest.city, // BLANK if ZIP available, else city name
    'Destination State*': dest.zip ? '' : dest.state, // BLANK if ZIP available, else state abbreviation  
    ...
    'Reference ID': `="` + uniqueRefId.slice(0, 8) + `"` // TEXT FORMAT - Our tracking ID (max 8 chars)
  };
}
