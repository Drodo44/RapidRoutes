// Utility: Normalize date to MM/DD/YYYY (DAT format)
export function normalizeDatDate(val) {
  ...
  if (!val) {
    ...
    if (mdy) {
      return `${pad2(mdy[1])}/${pad2(mdy[2])}/${mdy[3]}`;
    }
    // Handle ISO YYYY-MM-DD
    const iso = val.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (iso) {
      return `${pad2(iso[2])}/${pad2(iso[3])}/${iso[1]}`;
    }
    d = new Date(val);
  } else {
    d = new Date(val);
...
export const MIN_PAIRS_REQUIRED = 6; // Minimum pairs needed for valid CSV generation
...
export async function generateDatCsvRows(lane) {
  ...
  // Always re-normalize to MM/DD/YYYY immediately before validation
