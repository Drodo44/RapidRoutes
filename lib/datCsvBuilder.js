...
export async function generateDatCsvRows(lane) {
    ...
    try {
        ...
        for (const pair of validPairs) {
            try {
                ...
                for (const contactMethod of contactVariants()) {
                    const row = baseRowFrom(lane, normalizedPickup, normalizedDelivery, contactMethod, usedRefIds);
        ...
        rows.sort((a, b) => {
            const refA = a['Reference ID (unique per organization; max 8 chars)'] || '';
            const refB = b['Reference ID (unique per organization; max 8 chars)'] || '';
