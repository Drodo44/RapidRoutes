...
function baseRowFrom(lane, origin, dest, contact) {
  ...
  return {
    ...
    'Pickup Latest': pickupLatest, // ensure both dates are present, default to earliest if latest is empty
    ...
    'Private Network Rate': '', // MUST be empty per DAT requirements
    'Allow Private Network Booking': 'no',
    'Allow Private Network Bidding': 'no',
    'Use DAT Loadboard*': 'yes',
    'DAT Loadboard Rate': '', // MUST be empty per DAT requirements
    ...
    'Reference ID (unique per organization; max 8 chars)': '' // Fixed header name to match DAT_HEADERS
  };
}
