...
export async function planPairsForLane(lane, { preferFillTo10 = false } = {}) {
  ...
  try {
    ...
    // Handle case where cities can't be found in database
    console.error(`CRAWL ERROR: ${error.message || error}`);

    if (error.message && error.message.includes('not found in cities table')) {
      console.log(`🚨 CREATING FULLY SYNTHETIC LANE: ${lane.origin_city}, ${lane.origin_state} -> ${lane.dest_city}, ${lane.dest_state}`);
      
      // Define diverse KMA codes for better market coverage
      const originKMAs = ['AL_BIR', 'GA_ATL', 'FL_JAX', 'NC_RAL', 'TX_DAL'];
      const destKMAs = ['CA_LAX', 'IL_CHI', 'NY_NYC', 'TX_DAL', 'GA_ATL'];
