...
function baseRowFrom(lane, origin, dest, contact) {
  ...
  // Generate reference ID if not present (fallback for legacy lanes)
  const referenceId = lane.reference_id || `RR${String(lane.id).slice(-5).padStart(5, '0')}`;
  ...
  return {
    ...
    'Private Network Rate': '', // Optional: Leave blank (no rate specified)
    ...
    'DAT Loadboard Rate': '', // Optional: Leave blank (no rate specified)
    'Allow DAT Loadboard Booking': 'no', // Optional: yes/no
    'Use Extended Network': 'no', // Optional: yes/no - per DAT spec examples
    ...
    'Reference ID (unique per organization; max 8 chars)': referenceId.slice(0, 8) // Our tracking ID (max 8 chars)
  };
}
