...
export async function generateDatCsvRows(lane) {
    ...
    try {
        ...
        if (!pairs || pairs.length < MINIMUM_PAIRS) {
            const error = new Error(
                `CRITICAL: Failed to generate minimum ${MINIMUM_PAIRS} pairs for lane ${lane.id}.\n` +
                `Found: ${pairs?.length || 0} pairs (${(pairs?.length || 0) * ROWS_PER_PAIR} rows).\n` +
                `Required: ${MINIMUM_PAIRS} pairs (${MINIMUM_PAIRS * ROWS_PER_PAIR} rows).`
            );
            error.details = {
                lane_id: lane.id,
                found_pairs: pairs?.length || 0,
                found_rows: (pairs?.length || 0) * ROWS_PER_PAIR,
