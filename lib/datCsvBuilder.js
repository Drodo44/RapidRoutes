...
const USE_PRIVATE = "yes", PRIVATE_BOOK = "no", PRIVATE_BID = "no", USE_DAT = "yes", USE_EXT = "yes";

function randInt(min, max) { return Math.floor(min + Math.random() * (max - min + 1)); }
function ensureDateLike(s) { return s || ""; }

function resolveWeight(lane) {
  if (lane.randomize_weight) {
    const min = Number(lane.weight_min), max = Number(lane.weight_max);
    if (Number.isFinite(min) && Number.isFinite(max) && max >= min && min > 0) return String(randInt(min, max));
...
function cityObj(c) { return { city: c.city, state: c.state, zip: c.zip || null }; }
...
export function rowsFromBaseAndPairs(lane, baseOrigin, baseDest, pairs) {
  ...
  for (const p of pairs.slice(0, 10)) postings.push({ origin: cityObj(p.pickup), dest: cityObj(p.delivery) });
  const out = [];
  for (const post of postings) for (const cm of CONTACT_METHODS) out.push(rowForPosting(lane, post.origin, post.dest, cm));
  return out;
}

export async function planPairsForLane(lane, { preferFillTo10 = false } = {}) {
  const originStr = lane.origin ?? (lane.origin_city && lane.origin_state ? `${lane.origin_city}, ${lane.origin_state}` : "");
  const destStr = lane.destination ?? (lane.dest_city && lane.dest_state ? `${lane.dest_city}, ${lane.dest_state}` : "");
  ...
  const { baseOrigin, baseDest, pairs, shortfallReason, allowedDuplicates } = await generateCrawlPairs({
    origin: originStr, destination: destStr, equipment: equip, preferFillTo10,
  });
...
export function chunkRows(rows, max = 499) {
  const chunks = []; for (let i = 0; i < rows.length; i += max) chunks.push(rows.slice(i, i + max));
  return chunks;
}
