...
export function rowsFromBaseAndPairs(lane, baseOrigin, baseDest, pairs, preferFillTo10 = false, usedRefIds = new Set()) {
  ...
  // If we don't have enough pairs, fail loudly — the crawler must provide true, diverse candidates
  if (usePairs.length < targetPairs) {
    console.error(`� INSUFFICIENT PAIRS: preferFillTo10=${preferFillTo10} required=${targetPairs} found=${usePairs.length}`);
    console.error(`💥 Make sure generateIntelligentCrawlPairs returns ${targetPairs} distinct KMA candidates within 75 miles for this lane.`);
    throw new Error(`Insufficient geographic crawl pairs: need ${targetPairs}, found ${usePairs.length}`);
  }

  // Build postings: base posting + exact N crawl postings (no repeats)
  const postings = [
