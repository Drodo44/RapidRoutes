...
function validateEquipmentWeight(lane) {
    ...
    const ROWS_PER_LANE = TARGET_PAIRS * 2; // Each pair generates 2 rows (email + phone)
...
export async function generateDatCsvRows(lane) {
    ...
    try {
        ...
        // Get city pairs from FreightIntelligence using singleton
        const intelligence = getFreightIntelligence();
        
        // Generate pairs with guaranteed minimum 6
        monitor.log('info', `Generating pairs for lane ${lane.id}...`);
        ...
        try {
            result = await intelligence.generateDiversePairs({
                origin: { 
                    ...
                    zip: lane.origin_zip // Include ZIP for better accuracy
                },
                destination: { 
                    city: lane.dest_city, 
                    state: lane.dest_state,
                    zip: lane.dest_zip // Include ZIP for better accuracy
                },
                equipment: lane.equipment_code,
                preferFillTo10: true // Always try to maximize pairs
            });
        } catch (pairError) {
            monitor.log('error', `Failed to generate pairs for lane ${lane.id}:`, pairError);
            throw new Error(`Failed to generate pairs for lane ${lane.id}: ${pairError.message}`);
        }
        ...
        const validPairs = pairs.filter(pair => {
            ...
            const hasPickup = pair.pickup && pair.pickup.city && pair.pickup.state;
            const hasDelivery = pair.delivery && pair.delivery.city && pair.delivery.state;
        ...
        for (const pair of validPairs) {
            try {
                ...
                for (const contactMethod of contactVariants()) {
                    const row = baseRowFrom(lane, pair.pickup, pair.delivery, contactMethod, usedRefIds);
        ...
        rows.sort((a, b) => a['Reference ID'].localeCompare(b['Reference ID']));
...
function baseRowFrom(lane, origin, dest, contact, usedRefIds = new Set()) {
  ...
  return {
    ...
    'Use DAT Load Board*': 'yes', // Required: yes/no - showing on DAT loadboard
    'DAT Load Board Rate': '', // BLANK - DAT requirement  
    'Allow DAT Load Board Booking': '', // BLANK - DAT requirement
    ...
    'Reference ID (unique per organization; max 8 chars)': uniqueRefId.slice(0, 8) // Plain text format - DAT compatible (no Excel quotes)
  };
}
