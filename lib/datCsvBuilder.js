...
export async function planPairsForLane(lane, { preferFillTo10 = false, usedCities = new Set() } = {}) {
  ...
  // EMERGENCY: Use simple unique crawler to prevent massive duplication
  const crawl = await generateEmergencyUniquePairs(baseOrigin, baseDest, lane.equipment_code, usedCities);
  
  // Convert emergency format to expected format
  return {
