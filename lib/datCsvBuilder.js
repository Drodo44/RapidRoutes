...
export function rowsFromBaseAndPairs(lane, baseOrigin, baseDest, pairs, preferFillTo10 = false, usedRefIds = new Set()) {
  ...
  if (preferFillTo10) {
    ...
    // If we got fewer than 5 pairs, supplement with base cities (but keep the intelligent ones)
    ...
    // Take exactly the first 5 pairs (intelligent ones first, then fallbacks)
    const usePairs = intelligentPairs.slice(0, 5);
    console.log(`🧠 Using ${usePairs.filter(p => !p.synthetic).length} intelligent + ${usePairs.filter(p => p.synthetic).length} synthetic pairs`);
    ...
    console.log(`🎯 SMART: Created ${postings.length} postings (1 base + 5 pairs with intelligence preserved)`);
    ...
    console.log(`🚨 SMART RESULT: Generated ${rows.length} rows with intelligence preserved`);
    return rows;
  }
