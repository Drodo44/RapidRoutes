...
export const DAT_HEADERS = [
  ...
  'Comment',
  'Commodity'
  // Reference ID removed - not necessary
];
...
export async function planPairsForLane(lane, { preferFillTo10 = false } = {}) {
  ...
  // EMERGENCY FIX: Use simple emergency crawler for Fill-to-5
  const crawl = preferFillTo10 
    ? await emergencyGeneratePairs(baseOrigin, baseDest, preferFillTo10)
...
export function rowsFromBaseAndPairs(lane, baseOrigin, baseDest, pairs, preferFillTo10 = false) {
  // INTELLIGENT FREIGHT POSTING: 1 base + up to 5 intelligent pairs when filling
  // Each posting duplicated for 2 contact methods = up to 12 rows when filling
  const maxPairs = preferFillTo10 ? 5 : 3; // 5 pairs when filling, 3 normally
  ...
  ].slice(0, maxPostings); // Dynamic limit based on fill preference
