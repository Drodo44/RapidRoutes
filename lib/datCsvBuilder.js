  ...
  try {
    ...
    try {
      ...
      if (lane.reference_id) {
        console.log(`[PHASE9-ROW-DEBUG] Lane ${lane.id}: Has RR# ${lane.reference_id}, checking for saved city choices...`);
        try {
          ...
          if (!choicesError && choices && choices.length > 0) {
            savedCityChoices = choices;
            console.log(`[PHASE9-ROW-DEBUG] Lane ${lane.id}: Found ${choices.length} saved city choices`);
          }
        } catch (err) {
          console.warn(`[PHASE9-ROW-DEBUG] Lane ${lane.id}: Could not load saved city choices:`, err.message);
        }
      }
