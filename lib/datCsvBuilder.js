...
function validateEquipmentWeight(lane) {
  ...
  // Minimum 6 unique city pairs (no maximum limit)
  // This MIN_PAIRS_REQUIRED constant is used throughout the module
  const MIN_PAIRS_REQUIRED = 6;   // Base pair + 5 generated pairs minimum    // CRITICAL FIX: Provide fallbacks for missing randomization range
...
export async function generateDatCsvRows(lane) {
    ...
    try {
        ...
        // Validate pairs with completely unique cities
        // Each pickup and delivery city must be used exactly once
        ...
        const validPairs = pairs.filter(pair => {
            ...
            const pickupCity = pair.pickup?.city;
            const deliveryCity = pair.delivery?.city;
...
export function rowsFromBaseAndPairs(lane, baseOrigin, baseDest, pairs, preferFillTo10 = false, usedRefIds = new Set(), generatedRefIds = new Map()) {
  ...
  // Enforce completely unique cities for each pair
  // Use the module-level MIN_PAIRS_REQUIRED constant
  
  // Track used cities to prevent any reuse
  const usedPickupCities = new Set();
  const usedDeliveryCities = new Set();
  
  // First pair is always the entered cities
  usedPickupCities.add(`${baseOrigin.city}-${baseOrigin.state}`);
  usedDeliveryCities.add(`${baseDest.city}-${baseDest.state}`);
  
  // Filter to only pairs with completely unique cities AND unique KMAs
  const usedKmas = new Set();
  // Add base pair KMAs first
  if (baseOrigin.kma_code) usedKmas.add(baseOrigin.kma_code);
  if (baseDest.kma_code) usedKmas.add(baseDest.kma_code);

  const uniquePairs = filteredPairs.filter(pair => {
    const pickupKey = `${pair.pickup.city}-${pair.pickup.state}`;
    const deliveryKey = `${pair.delivery.city}-${pair.delivery.state}`;
    
    // Skip if we've used either city before
    if (usedPickupCities.has(pickupKey) || usedDeliveryCities.has(deliveryKey)) {
      return false;
    }
    
    // Skip if we've used either KMA before
    const pickupKma = pair.pickup.kma_code;
    const deliveryKma = pair.delivery.kma_code;
    if (usedKmas.has(pickupKma) || usedKmas.has(deliveryKma)) {
      return false;
    }
    
    // Mark these cities and KMAs as used
    usedPickupCities.add(pickupKey);
    usedDeliveryCities.add(deliveryKey);
    if (pickupKma) usedKmas.add(pickupKma);
    if (deliveryKma) usedKmas.add(deliveryKma);
    
    return true;
  });
  
  // Sort by diversity score
  uniquePairs.sort((a, b) => (b.score || 0) - (a.score || 0));
  
  // Validate we have minimum required pairs with unique cities & KMAs
  if (uniquePairs.length < MIN_PAIRS_REQUIRED - 1) { // -1 because base pair isn't in uniquePairs
    throw new Error(
      `Insufficient unique city pairs: Found ${uniquePairs.length} pairs with unique cities, ` +
      `minimum required is ${MIN_PAIRS_REQUIRED - 1} (plus 1 base pair)`
    );
  }
  
  // Create the final set of pairs with completely unique cities
  console.log(`✅ Found ${uniquePairs.length} pairs with unique cities`);
  
  // Log details about uniqueness
  console.log(`📍 Unique pickup cities: ${usedPickupCities.size}`);
  console.log(`🏢 Unique delivery cities: ${usedDeliveryCities.size}`);
  console.log(`🌐 Unique KMAs: ${usedKmas.size}`);
  
  // Final validation - must have minimum 6 unique KMAs
  if (usedKmas.size < MIN_PAIRS_REQUIRED) {
    throw new Error(
      `Insufficient KMA diversity: Found ${usedKmas.size} unique KMAs, ` +
      `minimum required is ${MIN_PAIRS_REQUIRED}`
    );
  }
  ...
  return [
    ...
    // Then all pairs with unique cities, sorted by diversity score
    ...uniquePairs
  ];
  ...
  // Final validation: Must have exactly 42 rows (21 pairs * 2 contact methods)
  if (rows.length !== 42) {
    throw new Error(
      `CRITICAL: Invalid row count for lane ${lane.id}. ` +
      `Generated ${rows.length} rows but DAT requires exactly 42 rows ` +
      `(1 base pair + 10 pickup crawls + 10 delivery crawls) * 2 contact methods.`
    );
  }
