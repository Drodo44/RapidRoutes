...
import { planPairsForLane } from './planPairsForLane.js';
...
export async function generateDatCsvRows(lane) {
    ...
    try {
        ...
        const { pairs } = await intelligence.generateLanePairs(lane);

        // Validate minimum requirement of 6 unique pairs
        if (pairs.length < 6) {
            throw new Error(`Could not generate minimum 6 pairs for lane ${lane.id}. Found: ${pairs.length}`);
        }

        // Log optimization info if we found more than minimum
        if (pairs.length > 6) {
            monitor.log('info', `Generated ${pairs.length} diverse pairs for lane ${lane.id} (minimum: 6)`);
        }
