...
export function ensureLaneWeightValidity(lane) {
  ...
  if (!rnd) {
    ...
    if (w <= 0) {
      throw new Error(`Invalid weight ${w} for lane ${lane.id}. Must be positive.`);
    }
    return;
  }
  
  // Randomized weights: validate min/max range
  const minWeight = Number(lane.weight_min);
  const maxWeight = Number(lane.weight_max);
  
  if (!Number.isFinite(minWeight) || minWeight <= 0) {
    throw new Error(`Invalid weight_min for lane ${lane.id}. Must be positive number.`);
  }
  if (!Number.isFinite(maxWeight) || maxWeight <= 0) {
    throw new Error(`Invalid weight_max for lane ${lane.id}. Must be positive number.`);
  }
  if (minWeight >= maxWeight) {
    throw new Error(`Invalid weight range for lane ${lane.id}. Min must be less than max.`);
  }
  
  // Randomized path: ensure min/max are valid and set fallbacks if needed
