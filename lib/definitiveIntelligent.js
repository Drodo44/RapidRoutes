  ...
  try {
    ...
    if (selectedPickups.length < targetPairs || selectedDeliveries.length < targetPairs) {
      ...
      if (selectedPickups.length < targetPairs) {
        ...
        const expandedPickupCandidates = uniquePickupCandidates
          .map(city => {
            const distance = calculateDistance(
              baseOrigin.latitude, baseOrigin.longitude,
              city.latitude, city.longitude
        ...
        const expandedIntelligentPickups = addFreightIntelligence(expandedPickupCandidates, baseOrigin);
      ...
      if (selectedDeliveries.length < targetPairs) {
        ...
        const expandedDeliveryCandidates = uniqueDeliveryCandidates
          .map(city => {
            const distance = calculateDistance(
              baseDest.latitude, baseDest.longitude,
              city.latitude, city.longitude
        ...
        const expandedIntelligentDeliveries = addFreightIntelligence(expandedDeliveryCandidates, baseDest);
