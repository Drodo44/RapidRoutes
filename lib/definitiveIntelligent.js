...
export async function generateDefinitiveIntelligentPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  ...
  try {
    ...
    // STEP 1: Get DAT-compatible pickup alternatives within 150 miles
    console.log(`🔍 Finding DAT-compatible pickup alternatives within 150 miles...`);
    const pickupCandidates = await getDatCompatibleCitiesWithinRadius(baseOrigin, 150, 50);
    console.log(`🔍 Found ${pickupCandidates.length} DAT-compatible pickup candidates`);

    // STEP 2: Get DAT-compatible delivery alternatives within 150 miles
    console.log(`🔍 Finding DAT-compatible delivery alternatives within 150 miles...`);
    const deliveryCandidates = await getDatCompatibleCitiesWithinRadius(baseDest, 150, 50);
    ...
    if (selectedPickups.length < targetPairs || selectedDeliveries.length < targetPairs) {
      ...
      if (selectedPickups.length < targetPairs) {
        console.log(`🔍 Expanding pickup search radius to 300 miles for missing ${targetPairs - selectedPickups.length} pickups...`);
        const expandedPickupCandidates = intelligentPickups
          ...
          .filter(city => city.distance_miles <= 300) // Expand to 300 miles
      ...
      if (selectedDeliveries.length < targetPairs) {
        console.log(`🔍 Expanding delivery search radius to 300 miles for missing ${targetPairs - selectedDeliveries.length} deliveries...`);
