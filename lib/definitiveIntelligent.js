...
export async function generateDefinitiveIntelligentPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  ...
  console.log(`📏 Distance limit: 75 miles standard, 100 miles maximum (rare worst-case)`);
  ...
  try {
    ...
    if (!originData?.[0]) {
      ...
      if (originVerification.verified && originVerification.data) {
        ...
        console.error(`❌ Could not find origin city ${origin.city}, ${origin.state} via HERE.com`);
        return {
    ...
    // STEP 1: Query pickup alternatives within 75 miles, different KMA, prioritizing verified cities
    console.log(`🔍 Querying pickup alternatives within 75 miles...`);
    ...
    console.log(`🔍 Querying delivery alternatives within 75 miles...`);
