...
// Test helper function for KMA capping logic
export function _capKMAForTest(candidates, maxPerKMA) {
  const byKMA = new Map();
  const result = [];
  
  for (const cand of candidates) {
    const kma = cand.cand.kma;
    const current = byKMA.get(kma) || [];
    if (current.length < maxPerKMA) {
      current.push(cand);
      byKMA.set(kma, current);
      result.push(cand);
    }
  }
