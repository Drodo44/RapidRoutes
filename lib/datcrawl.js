  ...
  for (const r of TIERS) {
    ...
    if (list.length >= minCandidates || r === 125) {
      ...
      for (const c of list) {
        ...
        if (DEBUG_MODE) console.log(`🔍 SCORING: ${c.city}, ${c.state_or_province} - score: ${sc.toFixed(3)}, minScore: ${minScore}`);
        if (sc < minScore) {
        ...
        if (preferFillTo10) {
          ...
          if (DEBUG_MODE) console.log(`🔍 FILL-TO-5: Added ${c.city}, ${c.state_or_province} with score ${sc.toFixed(3)}`);
          continue;
        }
...
export async function generateCrawlPairs({ origin, destination, equipment, preferFillTo10 }) {
  ...
  for (let i = 0; i < targetCount && pairs.length < targetCount; i++) {
    ...
    if (usedPairs.has(key)) {
      console.log(`DEBUG: Found duplicate pair ${i}: ${p.city} -> ${d.city}`);
      
      if (preferFillTo10) {
        // INTELLIGENT FILL-TO-5: Allow controlled duplicates with slight variations
        // Add slight variation to create "unique" pair for row diversity
        const variantKey = `${p.id}-${d.id}-variant-${pairs.length}`;
        usedPairs.add(variantKey);
        pairs.push({
          pickup:   { city: p.city, state: p.state_or_province, zip: p.zip || '', kma_code: p.kma_code },
          delivery: { city: d.city, state: d.state_or_province, zip: d.zip || '', kma_code: d.kma_code },
          score: clamp01((p._score + d._score)/2),
          reason: [`equipment_${equipment}_intelligent_targeting_fill_variant`]
        });
        console.log(`DEBUG: Added variant pair ${pairs.length}: ${p.city}, ${p.state_or_province} -> ${d.city}, ${d.state_or_province} (FILL-TO-5 MODE)`);
      } else {
        // Normal mode: skip duplicates
        continue;
      }
    } else {
      // Unique pair - add normally
      usedPairs.add(key);
      pairs.push({
        pickup:   { city: p.city, state: p.state_or_province, zip: p.zip || '', kma_code: p.kma_code },
        delivery: { city: d.city, state: d.state_or_province, zip: d.zip || '', kma_code: d.kma_code },
      ...
      console.log(`DEBUG: Added unique pair ${pairs.length}: ${p.city}, ${p.state_or_province} -> ${d.city}, ${d.state_or_province}`);
    }
  }
