...
async function pickSideCandidates(baseCity, equip){
  ...
  for (const r of TIERS) {
    ...
    if (list.length >= 12 || r === 125) {
      ...
      // SMART CANDIDATE SELECTION - Limit to top 5 for quality over quantity
      const candidates = Array.from(byKMA.values())
        .sort((a,b) => b._score - a._score)
        .slice(0, 5); // Changed from 10 to 5 for better quality
...
export async function generateCrawlPairs({ origin, destination, equipment, preferFillTo10 }) {
  ...
  // OPTIMIZED: 5 high-quality pairs instead of 10 for better freight intelligence
  const targetCount = 5;
