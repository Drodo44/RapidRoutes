  ...
  for (const r of TIERS) {
    ...
    if (list.length >= minCandidates || r === 125) {
      ...
      for (const c of list) {
        ...
        const minScore = preferFillTo10 ? 0.01 : 0.85; // Accept almost everything for Fill-to-5!
        ...
        // EMERGENCY FIX: For Fill-to-5, also store candidates by city to bypass KMA limits
        if (preferFillTo10) {
          const cityKey = `${c.city}-${c.state_or_province}`;
          if (!byKMA.has(cityKey)) {
            byKMA.set(cityKey, { ...c, _score: sc });
          }
        }
      }
...
export async function generateCrawlPairs({ origin, destination, equipment, preferFillTo10 }) {
  ...
  if (pairs.length < targetCount) {
    ...
    if (!preferFillTo10) {
      ...
      // Fill with ANY decent candidates (0.01+ to accept almost everything)
      const poolP = picksO.filter(c => c._score >= 0.01); // Accept almost everything
      const poolD = picksD.filter(c => c._score >= 0.01); // Accept almost everything
