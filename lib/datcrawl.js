  ...
  for (const r of TIERS) {
    ...
    if (list.length >= minCandidates || r === 125) {
      ...
      for (const c of list) {
        ...
        // MUCH more permissive scoring for Fill-to-5 mode
        const minScore = preferFillTo10 ? 0.50 : 0.85; // 50% threshold for Fill-to-5!
...
export async function generateCrawlPairs({ origin, destination, equipment, preferFillTo10 }) {
  ...
  if (pairs.length < targetCount) {
    ...
    if (!preferFillTo10) {
      ...
      // Fill with good candidates (0.40+ instead of 0.60+ for more options)
      const poolP = picksO.filter(c => c._score >= 0.40); // Much lower threshold
      const poolD = picksD.filter(c => c._score >= 0.40); // Much lower threshold
