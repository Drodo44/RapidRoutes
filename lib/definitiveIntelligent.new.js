...
export async function generateDefinitiveIntelligentPairs(origin, destination) {
  try {
    ...
    try {
      // PHASE 3 FIX: Use Promise.allSettled to prevent single city failure from breaking entire operation
      ...
      const originVerified = verificationResults[0].status === 'fulfilled' ? verificationResults[0].value : null;
      const destVerified = verificationResults[1].status === 'fulfilled' ? verificationResults[1].value : null;
      
      // Log partial failures but continue with available data
      if (verificationResults[0].status === 'rejected') {
        console.warn('Origin city verification failed:', verificationResults[0].reason?.message);
      }
      if (verificationResults[1].status === 'rejected') {
        console.warn('Destination city verification failed:', verificationResults[1].reason?.message);
      }
      ...
      if (originVerified) {
        await supabaseAdmin.from('city_verifications').upsert({
          city: baseOrigin.city,
    ...
    const { data: originAdjacent } = await supabaseAdmin
      .from('kma_adjacency')
      .select('adjacent_kma')
      .eq('kma_code', baseOrigin.kma_code);
      
    const { data: destAdjacent } = await supabaseAdmin
      .from('kma_adjacency')
      .select('adjacent_kma')
