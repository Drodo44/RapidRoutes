...
export async function generateAlternativeCitiesWithHERE(centerLat, centerLng, radiusMiles = 75, limit = 20) {
  ...
  try {
    ...
    // Use discover API with circle area constraint (without 'at' parameter)
    const url = `https://discover.search.hereapi.com/v1/discover?q=city&in=circle:${centerLat},${centerLng};r=${radiusMeters}&limit=${limit}&apiKey=${HERE_API_KEY}`;
