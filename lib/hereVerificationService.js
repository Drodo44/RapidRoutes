 ...
 * - Rate limitiexport async function generateAlternativeCitiesWithHERE(centerLat, centerLng, radiusMiles = 50, limit = 10) {
  if (!HERE_API_KEY) {
    console.warn('⚠️ HERE_API_KEY not configured - skipping alternative city generation');
    return []; // Return empty array when API not available
...
export async function generateAlternativeCitiesWithHERE(centerLat, centerLng, radiusMiles = 100, limit = 20) {
  ...
  try {
    ...
    // Use discover API instead of browse API with updated parameters
    const url = `https://discover.search.hereapi.com/v1/discover?at=${centerLat},${centerLng}&in=circle:${centerLat},${centerLng};r=${radiusMeters}&q=city&limit=${limit}&apiKey=${HERE_API_KEY}`;
