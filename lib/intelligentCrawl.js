...
async function getPreferredPickups(userId) {
  ...
  console.log(`📍 Loaded ${preferredPickups?.length || 0} preferred pickup locations for user`);
...
export async function generateIntelligentCrawlPairs({ 
  ...
  try {
    ...
    // Get user's preferred pickup locations
    const preferredPickups = userId ? await getPreferredPickups(userId) : [];
