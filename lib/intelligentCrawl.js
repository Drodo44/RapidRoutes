...
import { generateSimpleGuaranteedPairs } from './simpleGuaranteed.js';
  ...
  try {
    ...
    if (preferredPickups.length > 0) {
      ...
      if (matchingPickup) {
        ...
        // This is one of your preferred pickups - use simple guaranteed generation
        const result = await generateSimpleGuaranteedPairs({
          origin,
          destination,
          equipment,
          preferFillTo10,
    ...
    // FALLBACK: Use simple guaranteed system for all non-preferred origins
    console.log(`🗺️ Using simple guaranteed system with 150-mile rule`);
    const result = await generateSimpleGuaranteedPairs({
      origin,
      destination,
      equipment,
