...
class IntelligentCache {
  ...
  async getCachedPairs(origin, destination, equipment) {
    ...
    try {
      ...
      const { data: recentLanes, error: laneError } = await supabaseAdmin
        .from('lanes')
  ...
  .eq('lane_status', 'archive') // Only archived lanes
  ...
  async generateAndCachePairs(origin, destination, equipment, laneId = null) {
    ...
    try {
      ...
      // Import and use the correct intelligence function
      const { generateDiversePairs } = await import('./FreightIntelligence.js');
      
      // Generate intelligent freight pairs with minimum guarantee
      const intelligentPairs = await generateDiversePairs(origin, destination, equipment);

      const pairs = Array.isArray(intelligentPairs?.pairs) ? intelligentPairs.pairs : [];
  ...
  async generateBasicPairs(origin, destination, equipment) {
    try {
      ...
      const result = await generateGeographicCrawlPairs({ origin, destination, equipment, minimumPairs: 6 });
