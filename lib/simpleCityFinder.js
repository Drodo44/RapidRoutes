...
export async function findCitiesNear(baseCity, baseLat, baseLng, radiusMiles = 75, limit = 10) {
  console.log(`Finding cities near ${baseCity} within ${radiusMiles} miles`);
  
  // Step 1: Get cities from Supabase database
  const { data: supabaseCities, error } = await supabaseAdmin
    .from('cities')
    .select('*')
  ...
  if (validCities.length < limit) {
    ...
    try {
      ...
      for (const hereCity of hereCities) {
        ...
        const { data: kmaMatch } = await supabaseAdmin
          .from('cities')
