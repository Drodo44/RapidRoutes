    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          // Note: Learning metadata columns don't exist yet in production
...
export async function generateLearningIntelligentPairs(params) {
  ...
  try {
    // Step 1: First get base cities from database (with coordinates)
    console.log(`📍 Looking up base cities in database...`);
    ...
    // Use HERE.com to discover missing cities instead of failing
    let finalOrigin = baseOrigin;
    let finalDest = baseDest;
    
    if (!baseOrigin) {
      console.log(`🔍 Origin ${origin.city}, ${origin.state} not in database - geocoding via HERE.com first`);
      
      // Use HERE.com to geocode the missing city
      const { verifyCityWithHERE } = await import('./hereVerificationService.js');
      const geocodedOrigin = await verifyCityWithHERE(origin.city, origin.state, null);
      
      if (geocodedOrigin && geocodedOrigin.found) {
        // Add the geocoded city to our database
        const cityToAdd = {
          city: geocodedOrigin.city,
          state: geocodedOrigin.state, 
          latitude: geocodedOrigin.coordinates.lat,
          longitude: geocodedOrigin.coordinates.lng,
          zip: null,
          kma_code: null, // Will be determined later
          discovered_by: 'here_geocoding'
        };
        
        await addDiscoveredCitiesToDatabase([cityToAdd]);
        
        // Now look it up again
        const { data: newOriginData } = await supabase
          .from('cities')
          .select('city, state_or_province, zip, latitude, longitude, kma_code, kma_name')
          .ilike('city', origin.city)
          .ilike('state_or_province', origin.state)
        ...
        finalOrigin = newOriginData?.[0];
      }
    }
    
    if (!baseDest) {
      console.log(`🔍 Destination ${destination.city}, ${destination.state} not in database - geocoding via HERE.com first`);
      
      // Use HERE.com to geocode the missing city
      const { verifyCityWithHERE } = await import('./hereVerificationService.js');
      const geocodedDest = await verifyCityWithHERE(destination.city, destination.state, null);
      
      if (geocodedDest && geocodedDest.found) {
        // Add the geocoded city to our database
        const cityToAdd = {
          city: geocodedDest.city,
          state: geocodedDest.state,
          latitude: geocodedDest.coordinates.lat,
          longitude: geocodedDest.coordinates.lng,
          zip: null,
          kma_code: null, // Will be determined later
          discovered_by: 'here_geocoding'
        };
        ...
        // Now look it up again
        const { data: newDestData } = await supabase
          .from('cities')
          .select('city, state_or_province, zip, latitude, longitude, kma_code, kma_name')
          .ilike('city', destination.city)
          .ilike('state_or_province', destination.state)
        ...
        finalDest = newDestData?.[0];
      }
    }
    
    if (!finalOrigin || !finalDest) {
      console.warn(`⚠️ Could not discover missing cities via HERE.com - falling back to standard system`);
    ...
    console.log(`📍 Base cities resolved (including HERE.com discoveries)`);
    
    // Step 2: Discover additional pickup cities using coordinates
    console.log(`🔍 Learning Phase 1: Discovering additional pickup cities...`);
    const newPickupCities = await discoverAndLearnCities(finalOrigin, 100, 'pickup');
    
    // Step 3: Discover additional delivery cities using coordinates
    console.log(`🔍 Learning Phase 2: Discovering additional delivery cities...`);
    const newDeliveryCities = await discoverAndLearnCities(finalDest, 100, 'delivery');
    
    // Step 4: Now generate pairs using the enhanced database
