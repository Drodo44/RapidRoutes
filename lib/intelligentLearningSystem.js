...
export async function generateLearningIntelligentPairs(params) {
  ...
  try {
    ...
    if (!baseOrigin || !baseDest) {
      console.warn(`⚠️ Base cities not found in database - skipping learning phase`);
    ...
    console.log(`📍 Base cities found with coordinates`);
    ...
    const newPickupCities = await discoverAndLearnCities(baseOrigin, 100, 'pickup');
    ...
    const newDeliveryCities = await discoverAndLearnCities(baseDest, 100, 'delivery');
