...
async function updateLaneLegacyFields(laneId, fullPayload, fallbackPayload = null, organizationId = null) {
...
export async function archiveLaneCovered(laneId, coverageData, userId, organizationId) {
    try {
        ...
        let laneQuery = supabase
            ...
            .eq('id', laneId);
        if (organizationId) {
            laneQuery = laneQuery.eq('organization_id', organizationId);
        }
        const { data: lane, error: laneError } = await laneQuery.single();
 ...
 */
export async function markLaneGaveBack(laneId, reason, organizationId = null) {
    try {
        let query = supabase
            .from('lanes')
            .update({
