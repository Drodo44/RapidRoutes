...
function resolveSupabaseUrl() {
    ...
    if (!url) {
        console.warn('[Supabase Config] Missing Supabase URL - auth features will be unavailable');
        return '';
    }
...
function resolveAnonKey() {
    ...
    if (!key) {
        console.warn('[Supabase Config] Missing Supabase anon key - auth features will be unavailable');
        return '';
    }
...
let browserAuthAutoRefreshEnabled = true;
...
export function getBrowserSupabase() {
    ...
    if (!url || !anon) {
        console.warn('[Supabase Client] Cannot initialize - missing configuration');
        return null;
    }
    ...
    if (!browserClient) {
        browserClient = createClient(url, anon, {
            auth: {
                persistSession: true,
                autoRefreshToken: browserAuthAutoRefreshEnabled,
...
export function setBrowserSupabaseAuthAutoRefresh(enabled) {
    const shouldEnable = Boolean(enabled);
    browserAuthAutoRefreshEnabled = shouldEnable;

    if (!browserClient?.auth) return;

    try {
        if (shouldEnable && typeof browserClient.auth.startAutoRefresh === 'function') {
            browserClient.auth.startAutoRefresh();
        }
        if (!shouldEnable && typeof browserClient.auth.stopAutoRefresh === 'function') {
            browserClient.auth.stopAutoRefresh();
        }
    } catch (error) {
        console.error('[Supabase Client] Failed to toggle auto refresh', error);
    }
}
