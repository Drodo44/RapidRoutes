...
function validateFlexibleDate(value, fieldName) {
  ...
  if (legacyPattern.test(value) || isoDatePattern.test(value) || isoDateTimePattern.test(value)) {
    // Additional semantic check: Date.parse must succeed
    const time = Date.parse(value.length === 10 && value.includes('/') ? convertLegacyToISO(value) : value);
    if (isNaN(time)) {
...
export function validateLane(lane) {
  ...
  // Custom flexible date validation for pickup_earliest / pickup_latest
  if ('pickup_earliest' in lane) {
    const err = validateFlexibleDate(lane.pickup_earliest, 'pickup_earliest');
    if (err) errors.push(err);
  } else {
    errors.push('pickup_earliest is required but missing');
  }
  if (lane.pickup_latest) {
    const err = validateFlexibleDate(lane.pickup_latest, 'pickup_latest');
    if (err) errors.push(err);
  }

  // If both date strings parse, ensure chronological ordering if both present
  try {
    if (lane.pickup_earliest && lane.pickup_latest) {
      const earliestIso = lane.pickup_earliest.includes('/') ? convertLegacyToISO(lane.pickup_earliest) : lane.pickup_earliest;
      const latestIso = lane.pickup_latest.includes('/') ? convertLegacyToISO(lane.pickup_latest) : lane.pickup_latest;
      const e = Date.parse(earliestIso);
      const l = Date.parse(latestIso);
      if (!isNaN(e) && !isNaN(l) && l < e) {
        errors.push('pickup_latest must be same day or after pickup_earliest');
      }
    }
  } catch (_) {
    // Ignore ordering if parsing failed; individual format errors already recorded
  }
