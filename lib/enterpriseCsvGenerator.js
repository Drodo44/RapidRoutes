...
export const ENTERPRISE_CSV_CONFIG = {
  ...
  generation: {
    ...
    enableCaching: false // DISABLED: Force fresh generation to apply Referenced ID and Rate updates
  },
  verification: {
 ...
 */
function deepMergeConfig(defaultConfig, overrideConfig) {
  const result = { ...defaultConfig };
  
  for (const [key, value] of Object.entries(overrideConfig)) {
    if (value && typeof value === 'object' && !Array.isArray(value)) {
      result[key] = deepMergeConfig(defaultConfig[key] || {}, value);
    } else {
      result[key] = value;
    }
  }
...
export class EnterpriseCsvGenerator {
  constructor(config = {}) {
    this.config = deepMergeConfig(ENTERPRISE_CSV_CONFIG, config);
  ...
  async generate(lanes, options = {}) {
    ...
    try {
      ...
      if (process.env.TEST_MODE_SIMPLE_ROWS === '1') {
        // Provide minimal result structure without running full verification/finalization
        return {
          success: true,
          error: null,
          csv: { rows: generationResult.rows, header: DAT_HEADERS, csv: null },
          laneResults: generationResult.laneResults,
          generation: { rowsGenerated: generationResult.rows.length },
          verification: { skipped: true, reason: 'TEST_MODE_SIMPLE_ROWS' }
        };
      }
  ...
  async _preGenerationValidation(lanes, options) {
    ...
    try {
      ...
      for (let i = 0; i < lanes.length; i++) {
        const lane = lanes[i];
        // 🔍 EMERGENCY DEBUG: Log lane id and raw date fields before normalization
        console.log(`🔍 [PRE-NORMALIZATION] Lane ${lane.id || i}: pickup_earliest=${lane.pickup_earliest}, pickup_latest=${lane.pickup_latest}`);

        // 🚨 EMERGENCY FIX: Default null dates before normalization
        if (!lane.pickup_earliest) {
          const tomorrow = new Date();
          tomorrow.setDate(tomorrow.getDate() + 1);
          lane.pickup_earliest = tomorrow.toISOString();
          console.log(`🔧 [AUTO-DEFAULT] Lane ${lane.id || i}: set pickup_earliest to ${lane.pickup_earliest}`);
        }
        if (!lane.pickup_latest) {
          const dayAfterTomorrow = new Date();
          dayAfterTomorrow.setDate(dayAfterTomorrow.getDate() + 2);
          lane.pickup_latest = dayAfterTomorrow.toISOString();
          console.log(`🔧 [AUTO-DEFAULT] Lane ${lane.id || i}: set pickup_latest to ${lane.pickup_latest}`);
        }

        // Normalize dates to MM/DD/YYYY before validation
        if (lane.pickup_earliest) {
        ...
        // 🔍 EMERGENCY DEBUG: Log lane id and date fields after normalization
        console.log(`🔍 [POST-NORMALIZATION] Lane ${lane.id || i}: pickup_earliest=${lane.pickup_earliest}, pickup_latest=${lane.pickup_latest}`);
        ...
        try {
          ...
          if (error.isValidationError) {
            console.error(`❌ Lane ${lane.id || i} failed validation:`, error.message, error.details);
      ...
      // TRACE: Emit a concise summary of lane validation outcome
      try {
        const passed = lanes.length - validationErrors.length;
        console.log(`🧪 [VALIDATION TRACE] Passed=${passed} Failed=${validationErrors.length}`);
        if (validationErrors.length) {
          validationErrors.slice(0, 50).forEach((ve, idx) => {
            console.log(`  ❌ [VALIDATION FAIL ${idx}] laneId=${ve.details?.lane_id || ve.lane_id || 'unknown'} msg=${ve.message}`);
          });
        }
      } catch (traceErr) {
        console.log('⚠️ [VALIDATION TRACE] Summary failed:', traceErr.message);
      }
  ...
  async _generateCsvRows(lanes) {
    ...
    console.log(`[DEBUG] _generateCsvRows called with ${lanes.length} lanes:`, lanes.map(l => l.id));
    console.log(`[DEBUG] Config object:`, JSON.stringify(this.config, null, 2));
    const traceId = enterpriseMonitor.startOperation(phaseId, {
    ...
    try {
      // Lightweight test mode: bypass full intelligent pairing & DB access
      if (process.env.TEST_MODE_SIMPLE_ROWS === '1') {
        const simpleRows = [];
        const laneResults = [];
        for (const lane of lanes) {
          // Produce a minimal compliant set (12 rows = 6 pairs * 2 contact methods) using static cities
            const pickup = lane.origin_city || 'Cincinnati';
            const pickupState = lane.origin_state || 'OH';
            const dest = lane.dest_city || 'Chicago';
            const destState = lane.dest_state || 'IL';
            const weight = lane.weight_lbs || 40000;
            const lengthFt = lane.length_ft || 53;
            for (let i = 0; i < this.config.generation.minPairsPerLane; i++) {
              ['email', 'primary phone'].forEach(contact => {
                simpleRows.push({
                  'Pickup Earliest*': lane.pickup_earliest,
                  'Pickup Latest': lane.pickup_latest,
                  'Length (ft)*': String(lengthFt),
            ...
            laneResults.push({ success: true, lane, rowsGenerated: this.config.generation.minPairsPerLane * 2 });
        }
        enterpriseMonitor.endOperation(phaseId, { success: true, rows_generated: simpleRows.length, test_mode: true });
        return { rows: simpleRows, laneResults, generationErrors: [] };
      }
      ...
      console.log(`[DEBUG] Starting batching with maxConcurrentLanes: ${this.config.generation.maxConcurrentLanes}`);
      for (let i = 0; i < lanes.length; i += this.config.generation.maxConcurrentLanes) {
        ...
        console.log(`[DEBUG] Created batch ${concurrentBatches.length} with ${batch.length} lanes:`, batch.map(l => l.id));
        concurrentBatches.push(batch);
      }
      console.log(`[DEBUG] Total batches created: ${concurrentBatches.length}`);

      for (const [batchIndex, batch] of concurrentBatches.entries()) {
        console.log(`[DEBUG] Processing batch ${batchIndex} with ${batch.length} lanes`);
        ...
        const batchPromises = batch.map(async (lane, laneIndex) => {
          ...
          try {
              const rowsOrError = await generateDatCsvRows(lane);
              if (Array.isArray(rowsOrError)) {
                if (rowsOrError.length < this.config.generation.minPairsPerLane * 2) {
                  generationErrors.push({
                    lane_id: lane.id,
                    lane_index: globalLaneIndex,
                    error: `Insufficient rows generated: got ${rowsOrError.length}`,
                    details: {}
                  });
                  this.monitor.completeLaneProcessing(lane.id, laneStartTime, rowsOrError.length);
                  return {
                    lane_id: lane.id,
                    lane_index: globalLaneIndex,
                    rows: rowsOrError,
                  ...
                  this.transaction.addGeneratedRows(lane.id, rowsOrError);
                  this.monitor.completeLaneProcessing(lane.id, laneStartTime, rowsOrError.length);
                  return {
                    lane_id: lane.id,
                    lane_index: globalLaneIndex,
              ...
              } else if (rowsOrError && rowsOrError.error) {
                generationErrors.push({
                  lane_id: lane.id,
                  lane_index: globalLaneIndex,
                  error: rowsOrError.error,
                  details: rowsOrError.laneFailureLog || {}
                });
                this.monitor.completeLaneProcessing(lane.id, laneStartTime, 0);
                return {
                  lane_id: lane.id,
                  lane_index: globalLaneIndex,
                  rows: [],
                ...
                generationErrors.push({
                  lane_id: lane.id,
                  lane_index: globalLaneIndex,
                  error: 'Unknown error in pair generation',
                  details: {}
                });
                this.monitor.completeLaneProcessing(lane.id, laneStartTime, 0);
                return {
