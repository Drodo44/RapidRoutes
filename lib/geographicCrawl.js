    ...
    for (const hereCity of hereCities) {
      ...
      if (verification.verified) {
        ...
        if (!existing?.length) {
          ...
          // Assign KMA based on nearest existing city with KMA
          let assignedKMA = null;
          const { data: nearestKMA } = await adminSupabase
            .from('cities')
            .select('kma_code')
            .not('kma_code', 'is', null)
            .order('location', { ascending: true }) // Use PostGIS if available, or fallback to coordinate math
            .limit(1);
          
          if (nearestKMA?.length > 0) {
            assignedKMA = nearestKMA[0].kma_code;
          }
          ...
          if (newCity) {
            await adminSupabase
              ...
              .insert({
                ...
                discovery_context: 'geographic_crawl'
              });
          }
        }
      }
