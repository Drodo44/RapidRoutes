...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  try {
    ...
    if (preferFillTo10 && (pickupAlternatives.length === 0 || deliveryAlternatives.length === 0)) {
      ...
      for (let i = 0; i < targetPairs; i++) {
        const pickupIndex = i % Math.max(1, pickupAlternatives.length);
        const deliveryIndex = i % Math.max(1, deliveryAlternatives.length);
        const pickup = pickupAlternatives[pickupIndex] || pickupAlternatives[0];
        const delivery = deliveryAlternatives[deliveryIndex] || deliveryAlternatives[0];
        
        if (!pickup || !delivery) {
          console.error(`🚨 MISSING DATA: pickup=${!!pickup}, delivery=${!!delivery} at index ${i}`);
          continue;
        }
