...
import { validatePairing, normalizePairing } from './validatePairings';
import { adminSupabase } from '../utils/supabaseClient';

// Optional imports for enhanced functionality
let findDiverseCities, enrichCityData, discoverNearbyCities, advancedCityDiscovery, findBestKMA;
try {
  ({ findDiverseCities } = await import('./improvedCitySearch'));
  ({ enrichCityData, discoverNearbyCities } = await import('./cityEnrichment'));
  ({ advancedCityDiscovery } = await import('./hereAdvancedServices'));
  ({ findBestKMA } = await import('./kmaAssignment'));
} catch (error) {
  console.warn('Some enhanced features not available:', error.message);
}
...
export async function generateGeographicCrawlPairs(origin, dest, radiusMiles = 75) {
  ...
  if (!originCities?.length) {
    throw new Error(`No KMAs found within ${radiusMiles} miles of origin ${origin.city}, ${origin.state}`);
  }
  ...
  if (!destCities?.length) {
    throw new Error(`No KMAs found within ${radiusMiles} miles of destination ${dest.city}, ${dest.state}`);
  }
  ...
  if (pairs.length === 0) {
    throw new Error('No valid pairs could be generated - check KMA assignments');
  }
