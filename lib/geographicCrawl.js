...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  try {
    ...
    if (preferFillTo10 && (pickupAlternatives.length === 0 || deliveryAlternatives.length === 0)) {
      ...
      // Generate EXACTLY targetPairs using intelligent selection WITH UNIQUE COMBINATIONS
      const usedCombinations = new Set();
      let attempts = 0;
      const maxAttempts = targetPairs * 5; // Prevent infinite loops
      
      while (pairs.length < targetPairs && attempts < maxAttempts) {
        attempts++;
        
        // Smart combination selection - avoid repeating pairs
        const pickupIndex = Math.floor(Math.random() * expandedPickups.length);
        const deliveryIndex = Math.floor(Math.random() * expandedDeliveries.length);
        ...
        console.log(`🗺️ UNIQUE PAIR ${pairs.length}/${targetPairs}: ${pickup.city}, ${pickup.state_or_province || pickup.state} -> ${delivery.city}, ${delivery.state_or_province || delivery.state}`);
