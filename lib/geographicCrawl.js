...
export async function generateGeographicCrawlPairs(origin, dest, radiusMiles = 75) {
  ...
  if (isLargeState && largeStateCode) {
    ...
    console.log(`${stateEmoji} ${largeStateCode} lane detected - loading ALL ${largeStateCode} cities statewide`);
    ...
    if (stateError) {
      ...
      destCities = allStateCities.map(city => ({
        ...city,
      ...
      console.log(`✅ Loaded ${destCities.length} ${largeStateCode} cities for state-wide posting`);
    }
  } else {
