...
export async function generateGeographicCrawlPairs(origin, dest, radiusMiles = 75) {
  ...
  const destState = dest.state || dest.state_or_province;
  ...
  if (isNewEngland(destState)) {
    ...
    destCities = destCities.filter(c => isNewEngland(c.state || c.state_or_province));
  ...
  destCities.sort((a, b) => {
    const aState = a.state || a.state_or_province;
    const bState = b.state || b.state_or_province;
