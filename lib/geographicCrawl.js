...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  try {
    ...
    // Strategy: Each pickup alternative goes to a different delivery alternative
    const maxRealPairs = Math.min(pickupAlternatives.length, deliveryAlternatives.length);
    
    for (let i = 0; i < maxRealPairs; i++) {
      const pickup = pickupAlternatives[i];
      const delivery = deliveryAlternatives[i];
    ...
    console.log(`✅ GEOGRAPHIC: Generated ${pairs.length} real geographic pairs using 75-mile rule (NO SYNTHETIC NEEDED)`);
