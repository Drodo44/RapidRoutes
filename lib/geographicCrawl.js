...
export async function generateGeographicCrawlPairs({ origin, destination, equipment }) {
  ...
  try {
    ...
    // Step 5: Guarantee minimum 5 unique KMAs (absolute requirement)
    if (allKMAs.size < 5) {
      throw new Error(`CRITICAL: Only found ${allKMAs.size} unique KMAs, need minimum 5. This should never happen in U.S. markets.`);
    }
