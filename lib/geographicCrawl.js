...
async function getCityCoords(city, state, label = "", zip = null) {
  ...
  // 4) Cross-state fuzzy (fallback of last resort)
  try {
    const cityPrefix = (city || '').split(/\s+/)[0];
    if (cityPrefix) {
  ...
  // 5) As a last resort, try HERE if key present
  if (HERE_API_KEY) {
  ...
  // No Supabase match and HERE not available
  console.error(`[GEOCRAWL] ${label} not found and HERE unavailable: ${city}, ${state}${zip ? ` (${zip})` : ''}`);
  return null;
}
