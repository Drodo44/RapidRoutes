...
export async function generateGeographicCrawlPairs(origin, dest, radiusMiles = 75) {
  ...
  // Special handling for Florida lanes - return ALL FL cities statewide
  if (destState === 'FL' || originState === 'FL') {
    console.log(`🌴 Florida lane detected - loading ALL FL cities statewide`);
    const { data: allFLCities, error: flError } = await supabaseAdmin
      .from('cities')
      .select('*')
    ...
    if (flError) {
      ...
      console.log(`✅ Loaded ${destCities.length} FL cities for state-wide posting`);
    }
  } else {
  ...
  // For FL lanes, keep only FL cities in destination list
  if (destState === 'FL' || originState === 'FL') {
    destCities = destCities.filter(c => normalizeState(c.state || c.state_or_province) === 'FL');
    console.log(`🌴 Filtered to ${destCities.length} FL-only destination cities`);
  }
