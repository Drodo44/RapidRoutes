...
export async function generateGeographicCrawlPairs(origin, dest, radiusMiles = 75) {
  ...
  for (const orig of originCities) {
    for (const dest of destCities) {
      ...
      // Create unique key based on city names to allow multiple pairs within same KMA
      const cityPairKey = `${orig.city},${orig.state || orig.state_or_province}-${dest.city},${dest.state || dest.state_or_province}`;
      if (seenPairs.has(cityPairKey)) continue;
  ...
  debug(`Generated ${pairs.length} pairs with ${seenPairs.size} unique city pair combinations`);
  
  // Count unique KMAs for validation (different from city pairs)
  const uniqueKMAs = new Set();
  for (const pair of pairs) {
    uniqueKMAs.add(`${pair.origin.kma_code}-${pair.destination.kma_code}`);
  }
  ...
  if (uniqueKMAs.size < MIN_UNIQUE_KMAS) {
    ...
    if (pairs.length >= MIN_PAIRS_OVERRIDE) {
      console.warn(`⚠️ GEOCRAWL WARNING: Low KMA diversity (${uniqueKMAs.size}/${MIN_UNIQUE_KMAS}) but sufficient pairs (${pairs.length}). Proceeding.`, {
      ...
      const errorMsg = `Insufficient unique KMAs: ${uniqueKMAs.size} (minimum required: ${MIN_UNIQUE_KMAS})`;
      console.error(`❌ GEOCRAWL ERROR: ${errorMsg}`, {
        uniqueKmas: uniqueKMAs.size,
        totalPairs: pairs.length,
        origin: `${origin.city}, ${origin.state}`,
    ...
    console.log(`✅ GEOCRAWL SUCCESS: Generated ${pairs.length} pairs with ${uniqueKMAs.size} unique KMAs`);
  }
  
  return { pairs, uniqueKmas: uniqueKMAs.size };
}
