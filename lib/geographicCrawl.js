...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10 = false, usedCities = new Set() }) {
  try {
    const RADIUS_MILES = 75;  // DAT standard radius
    const MIN_UNIQUE_KMAS_PER_SIDE = 6;  // Absolute minimum, will get more if available
    ...
    // Find ALL cities within 75 miles - NO LIMIT, get every possible unique KMA
    // Business requirement: Never exceed 75 miles for freight intelligence
    const pickupAlternatives = await findCitiesNearLocation(baseOrigin, 999999, 75, usedCities, equipment, false);
    const deliveryAlternatives = await findCitiesNearLocation(baseDest, 999999, 75, usedCities, equipment, false);
