...
function calculateFreightIntelligence(cityRow, equipment, baseCity) {
  ...
  // GEORGIA/SOUTH CAROLINA FREIGHT CORRIDO        uniquePickupKmas, 
        uniqueDeliveryKmas,
    ...
    console.error('❌ GEOGRAPHIC CRAWL FAILED:', error);
    ...
    console.error(`   Target: ${preferFillTo10 ? '10' : '5'} pairs`);
    
    throw new Error(`Geographic crawl failed for ${origin.city}, ${origin.state} → ${destination.city}, ${destination.state}: ${error.message}`);
  }ate === 'ga' && baseName.includes('augusta')) || (baseState === 'sc' && baseName.includes('aiken'))) {
...
async function generateIntelligentFallbackPairs({ baseOrigin, baseDest, equipment, needed, existingPairs, usedCities }) {
  ...
  const RELEVANCE_STANDARDS = {
    ...
    minIntelligenceScore: 0.01, // Lower threshold - accept more pairs
    maxFallbackPairs: 10,       // Allow UNLIMITED fallback generation to guarantee results
  ...
  if (fallbackPairs.length < needed) {
    ...
    // Find cities in different KMAs but within expanded freight distance (100 miles worst-case)
    const adjacentPickups = await findCitiesNearLocation(baseOrigin, needed * 3, 100, usedCities, equipment, true);
    const adjacentDeliveries = await findCitiesNearLocation(baseDest, needed * 3, 100, usedCities, equipment, true);
...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  try {
    const TARGET_PAIRS = 6; // FIXED: Always generate 6 pairs (1 base + 5 additional) × 2 contacts = 12 rows
    console.log(`🗺️ GEOGRAPHIC CRAWL: ${origin.city}, ${origin.state} -> ${destination.city}, ${destination.state} (Target: ${TARGET_PAIRS} pairs)`);
    ...
    // CRITICAL: Must generate 6 pairs minimum (12 rows with both contact methods)
    const MIN_REQUIRED_PAIRS = 6;
    const targetPairs = preferFillTo10 ? 10 : MIN_REQUIRED_PAIRS;
    let pickupAlternatives = [];
    let deliveryAlternatives = [];
    let logMessage = '';
    
    console.log(`🎯 PAIRS TARGET: ${targetPairs} (minimum required: ${MIN_REQUIRED_PAIRS})`);
    ...
    if (pickupKmas75 >= targetPairs && deliveryKmas75 >= targetPairs) {
      ...
      // Step 2: Extended KMA Search (100 miles worst-case, different KMA) - Extend radius to find unique KMAs
      logMessage = ' (Extended KMA Search: 100mi radius worst-case)';
      console.warn(`⚠️ EXTENDING SEARCH: Need more unique KMAs. Expanding to 100 miles while maintaining KMA diversity.`);
      
      const p_extended = await findCitiesNearLocation(baseOrigin, targetPairs * 2, 100, usedCities, equipment, false);
      const d_extended = await findCitiesNearLocation(baseDest, targetPairs * 2, 100, usedCities, equipment, false);
      ...
      console.log(`📊 100-mile KMA diversity: ${pickupKmas125} pickup KMAs, ${deliveryKmas125} delivery KMAs`);
      
      if (pickupKmas125 < targetPairs || deliveryKmas125 < targetPairs) {
        // Step 3: Final fallback - Allow same KMA within 100 miles (violates KMA diversity but ensures lane generation)
        logMessage = ' (Fallback: 100mi radius worst-case, same KMA allowed)';
        console.warn(`🚨 FALLBACK MODE: Insufficient unique KMAs even at 100 miles. Allowing same KMA to complete lane generation.`);
        
        const p_fallback = await findCitiesNearLocation(baseOrigin, targetPairs, 100, usedCities, equipment, true);
        const d_fallback = await findCitiesNearLocation(baseDest, targetPairs, 100, usedCities, equipment, true);
        ...
        console.log(`✅ KMA DIVERSITY ACHIEVED: Found sufficient unique KMAs at extended 100-mile radius`);
      }
    }
    ...
    // BROKER-INTELLIGENT KMA SELECTION: Ensure minimum required pairs with KMA diversity
    const pairs = [];

    // Enhanced KMA selection: pick the BEST city from each unique KMA, prioritizing freight intelligence score
    function selectBestCityPerKma(cityList, requiredCount) {
      // Ensure we always get at least MIN_REQUIRED_PAIRS
      const limit = Math.max(requiredCount, MIN_REQUIRED_PAIRS);
    ...
    // Track pair generation results for enhanced logging
    const resultSummary = `Generated ${finalPairs.length}/${target} pairs with ${uniquePickupKmas} pickup KMAs and ${uniqueDeliveryKmas} delivery KMAs`;
    
    if (finalPairs.length >= MIN_REQUIRED_PAIRS) {
      if (finalPairs.length >= target) {
        console.log(`✅ OPTIMAL SUCCESS: ${resultSummary}`);
      } else {
        console.log(`✅ SUFFICIENT SUCCESS: ${resultSummary} (exceeds minimum ${MIN_REQUIRED_PAIRS})`);
      }
    } else {
      console.log(`❌ INSUFFICIENT PAIRS: ${resultSummary} (below minimum ${MIN_REQUIRED_PAIRS})`);
      throw new Error(`Failed to generate minimum ${MIN_REQUIRED_PAIRS} pairs (got ${finalPairs.length})`);
    }
    ...
    return {
      ...
      kmaAnalysis: {
        required: target,
        achieved: finalPairs.length,
        searchRadius: 75, // Default to standard radius
        uniquePickupKmas,
        uniqueDeliveryKmas,
        success: finalPairs.length >= target,
        fallbackUsed: finalPairs.length > filteredPairs.length
    ...
    console.error('❌ CRITICAL ERROR IN GEOGRAPHIC CRAWL:');
    console.error(`   Origin: ${origin.city}, ${origin.state}`);
    console.error(`   Dest: ${destination.city}, ${destination.state}`);
    console.error(`   Equipment: ${equipment}`);
    console.error(`   Target: ${preferFillTo10 ? '10' : '5'} pairs`);
