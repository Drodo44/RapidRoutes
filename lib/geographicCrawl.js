...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10 = false, usedCities = new Set() }) {
  try {
    ...
    // Find MANY cities within 100 miles MAX to ensure we get minimum 6 unique KMAs per side
    // Search for MORE cities to increase KMA diversity chances
    const pickupAlternatives = await findCitiesNearLocation(baseOrigin, MIN_UNIQUE_KMAS_PER_SIDE * 5, 100, usedCities, equipment, false);
    const deliveryAlternatives = await findCitiesNearLocation(baseDest, MIN_UNIQUE_KMAS_PER_SIDE * 5, 100, usedCities, equipment, false);
...
async function findCitiesNearLocation(baseCity, targetCount = 6, maxDistanceMiles = 75, usedCities = new Set(), equipment = null, allowSameKma = false) {
  ...
  // ENHANCED SELECTION: KMA diversity FIRST, distance SECOND (within 100 miles MAX)
  ...
  // Group valid cities by KMA (all within 100 miles already)
