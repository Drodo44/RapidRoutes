...
// Rank pairings with freight intelligence and normalize output
function rankPairingsWithFreightIntelligence(pickups, deliveries, equipment) {
  ...
  for (const pickup of pickups) {
    for (const delivery of deliveries) {
      if (pickup.kma_code !== delivery.kma_code) { // Don't pair same KMA
        ...
        if (!pickup.city || !pickup.state_or_province || !pickup.kma_code || 
            !delivery.city || !delivery.state_or_province || !delivery.kma_code) {
          console.warn('⚠️ Skipping pair due to incomplete data:', { pickup: pickup.city, delivery: delivery.city });
          continue;
        }
        
        const distance = calculateDistance(pickup.latitude, pickup.longitude, delivery.latitude, delivery.longitude);
        
        // Skip pairs over 100 miles (absolute rule)
        if (distance > 100) continue;
        
        // Freight intelligence scoring
        const score = calculateFreightScore(pickup, delivery, distance, equipment);
        
        // Normalize pairing output format
  ...
  allPairs.sort((a, b) => b.freight_score - a.freight_score);
  
  console.log(`🎯 Ranked ${allPairs.length} normalized pairs by freight intelligence`);
  return allPairs;
}
...
export async function generateGeographicCrawlPairs(originCity, originState, destCity, destState, equipment = 'V') {
  console.log(`🎯 INTELLIGENCE: Starting pairing for ${originCity}, ${originState} → ${destCity}, ${destState}`);
  
  try {
    // Step 1: Get base coordinates for origin and destination
    const baseOrigin = await getCityCoords(originCity, originState, "Origin");
    if (!baseOrigin) throw new Error(`Origin not found: ${originCity}, ${originState}`);

    const baseDest = await getCityCoords(destCity, destState, "Destination");
    if (!baseDest) throw new Error(`Destination not found: ${destCity}, ${destState}`);
    ...
    // Step 4: If fewer than 5 unique KMAs, call HERE API and merge results
    let allPickups = [...supabasePickups];
    let allDeliveries = [...supabaseDeliveries];
    
    if (allKMAs.size < 5) {
      console.log(`🎯 Need more KMAs (${allKMAs.size}/5), calling HERE API...`);
    ...
    // Step 5: If still <5 KMAs, use Supabase fuzzy fallback within 100 miles
    if (allKMAs.size < 5) {
      console.log(`🎯 Still need more KMAs (${allKMAs.size}/5), using fuzzy fallback...`);
      
      // Get additional cities from broader Supabase search within 100 miles
      const { data: extraPickups } = await adminSupabase.rpc("find_cities_within_radius", {
        lat_param: baseOrigin.latitude,
        lng_param: baseOrigin.longitude,
        radius_meters: 100 * 1609.34, // 100 miles
      });
      
      const { data: extraDeliveries } = await adminSupabase.rpc("find_cities_within_radius", {
        lat_param: baseDest.latitude,
        lng_param: baseDest.longitude,
        radius_meters: 100 * 1609.34, // 100 miles
      });
      
      if (extraPickups) allPickups = mergeAndDeduplicateCities(allPickups, extraPickups.filter(c => c.kma_code));
      if (extraDeliveries) allDeliveries = mergeAndDeduplicateCities(allDeliveries, extraDeliveries.filter(c => c.kma_code));
      
      // Update KMA count after fuzzy fallback
      allKMAs.clear();
      allPickups.forEach(c => c.kma_code && allKMAs.add(c.kma_code));
      allDeliveries.forEach(c => c.kma_code && allKMAs.add(c.kma_code));
      
      console.log(`🎯 After fuzzy fallback: ${allPickups.length} pickup cities, ${allDeliveries.length} delivery cities, ${allKMAs.size} unique KMAs`);
    }

    // Step 6: Guarantee at least 5 unique KMAs (absolute requirement)
    if (allKMAs.size < 5) {
      throw new Error(`CRITICAL: Only found ${allKMAs.size} unique KMAs within 100mi, need minimum 5. This should never happen in U.S. markets.`);
    }

    // Step 7: Create unique pickup/delivery lists by KMA (no duplicates)
    ...
    // Step 8: Rank final pairings with freight intelligence (normalized output)
    const rankedPairs = rankPairingsWithFreightIntelligence(uniquePickups, uniqueDeliveries, equipment);
    
    // Step 9: Ensure we have at least 5 valid pairings
    if (rankedPairs.length < 5) {
      throw new Error(`CRITICAL: Only generated ${rankedPairs.length} valid pairings, need minimum 5.`);
    }
    
    console.log(`🎯 INTELLIGENCE: Generated ${rankedPairs.length} normalized pairs with ${allKMAs.size} unique KMAs`);
