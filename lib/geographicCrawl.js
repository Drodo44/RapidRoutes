...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  try {
    ...
    if (preferFillTo10 && filteredPairs.length !== 5) {
      console.error(`🚨 CRITICAL ERROR: Fill-to-10 mode requires exactly 5 pairs, got ${filteredPairs.length} after filtering`);
      throw new Error(`Geographic crawl guarantee failed: got ${filteredPairs.length} pairs after Long Island filter, expected 5 for fill-to-10 mode`);
    };
    ...
    // Return the filtered pairs (Long Island removed) so callers cannot bypass the ban
    return {
      baseOrigin: {
        city: baseOrigin.city,
        state: baseOrigin.state_or_province,
