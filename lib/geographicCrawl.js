...
async function findCitiesNearLocation(baseCity, targetCount = 5, maxDistanceMiles = 75, usedCities = new Set(), equipment = null) {
  ...
  for (const city of nearbyCities) {
    ...
    // NORMALIZE state detection: accept two-letter or full state name (e.g. 'NY' or 'New York')
    const isNewYorkState = stateName === 'ny' || stateName.includes('new york');

    // LONG ISLAND ABSOLUTE BAN - Truckers avoid islands due to bridge/tunnel/ferry constraints
    // Match common Long Island city names OR a conservative bounding box that covers Long Island
    const inLongIslandBBox = (typeof city.longitude === 'number' && typeof city.latitude === 'number') &&
      (city.longitude > -74.5 && city.longitude < -71.5) && (city.latitude > 40.0 && city.latitude < 41.6);
    ...
    if (isNewYorkState && (
        ...
        // Final fallback: bounding-box test for Long Island
        inLongIslandBBox
    )) {
      logisticsPenalty = -99.0; // ABSOLUTE BAN - Never select Long Island cities
      console.log(`🚫 ABSOLUTE BAN: ${cityKey} is detected as Long Island - COMPLETELY AVOIDED`);
    }
  ...
  let selectedCities = citiesWithDistance
    .sort((a, b) => b.totalScore - a.totalScore) // Higher score is better
    .slice(0, targetCount);

  // FINAL SAFETY FILTER: remove any cities that match Long Island ban criteria (extra protection)
  selectedCities = selectedCities.filter(city => {
    const cName = city.city.toLowerCase();
    const sName = city.state_or_province.toLowerCase();
    const isNY = sName === 'ny' || sName.includes('new york');
    const inLIBox = (typeof city.longitude === 'number' && typeof city.latitude === 'number') &&
      (city.longitude > -74.5 && city.longitude < -71.5) && (city.latitude > 40.0 && city.latitude < 41.6);
    const liMatch = isNY && (cName.includes('montauk') || cName.includes('hempstead') || cName.includes('babylon') || inLIBox);
    if (liMatch) console.log(`🚫 FINAL FILTER: Excluding Long Island city from selection: ${city.city}, ${city.state_or_province}`);
    return !liMatch;
  });
