...
async function findNearbyKMAs(lat, lon, radius = 75) {
  ...
  let maxRadius = 100;  // Limit to 100 miles as required
...
export async function generateGeographicCrawlPairs(origin, dest, radiusMiles = 75) {
  ...
  // Validate minimum KMA requirements - MUST have at least 5 unique KMAs
  if (seenPairs.size < 5) {
    const errorMsg = `Insufficient unique KMAs: ${seenPairs.size} (minimum required: 5)`;
    console.error(`❌ GEOCRAWL ERROR: ${errorMsg}`, {
