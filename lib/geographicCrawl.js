...
export async function generateGeographicCrawlPairs(origin, dest, radiusMiles = 75) {
  ...
  if (seenPairs.size < MIN_UNIQUE_KMAS) {
    ...
    // FIX: Fallback for production - don't throw error if we have at least 3 KMAs
    // This ensures the API doesn't fail but logs the warning
    if (seenPairs.size >= 3) {
      console.warn(`⚠️ GEOCRAWL WARNING: Only ${seenPairs.size} unique KMAs found, but proceeding with fallback`);
    } else {
      throw new Error(errorMsg);
    }
  } else {
