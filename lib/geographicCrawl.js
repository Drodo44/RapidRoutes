...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  try {
    ...
    if (preferFillTo10 && (pickupAlternatives.length === 0 || deliveryAlternatives.length === 0)) {
      ...
      // Generate real geographic pairs - FORCE GUARANTEE exactly targetPairs
      console.log(`🔧 PAIR GENERATION: Need ${targetPairs} pairs, have ${pickupAlternatives.length} pickup alternatives, ${deliveryAlternatives.length} delivery alternatives`);
      
      // NUCLEAR FIX: Always use base cities as fallback to guarantee pairs
      ...
      for (let i = 0; i < targetPairs; i++) {
        ...
        // Try to use alternatives first, fallback to base cities
        if (pickupAlternatives.length > 0) {
          const pickupIndex = i % pickupAlternatives.length;
          pickup = pickupAlternatives[pickupIndex];
        } else {
        ...
        if (deliveryAlternatives.length > 0) {
          ...
          delivery = deliveryAlternatives[deliveryIndex];
        } else {
          delivery = fallbackDelivery;
        }
        
        // GUARANTEE: Always create a pair
        pairs.push({
          pickup: {
            city: pickup.city,
        ...
        console.log(`🗺️ PAIR ${i+1}/${targetPairs}: ${pickup.city}, ${pickup.state_or_province} -> ${delivery.city}, ${delivery.state_or_province}`);
