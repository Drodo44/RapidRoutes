...
async function findCitiesNearLocation(baseCity, targetCount = 6, maxDistanceMiles = 75, usedCities = new Set(), equipment = null, allowSameKma = false) {
    ...
    for (const hereCity of hereCities) {
      ...
      if (verification.verified) {
        ...
        if (!existing?.length) {
          ...
          // Assign KMA based on nearest existing city with KMA using distance calculation
          ...
          const { data: newCity } = await adminSupabase
            .from('cities')
            .insert({
              city: hereCity.city,
          ...
          if (newCity) {
            await adminSupabase
              ...
              .insert({
                ...
                discovery_context: `geographic_crawl_nearest_${minDistance.toFixed(1)}mi`
              });
          }
        } else {
          // City exists, make sure it has proper KMA and HERE verification
          const existingCity = existing[0];
          if (!existingCity.kma_code) {
            console.log(`🔧 Updating existing city with KMA: ${hereCity.city}, ${hereCity.state}`);
            // Find nearest KMA for existing city that lacks one
            let assignedKMA = null;
            let minDistance = Infinity;
  ...
  // Return best scoring cities with KMA UNIQUENESS filtering
  ...
  // Sort by score and select cities with unique KMAs
  const sortedCities = validCities.sort((a, b) => b.totalScore - a.totalScore);
