...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10 = false, usedCities = new Set() }) {
  try {
    // INTELLIGENCE TARGET: Find ALL unique KMAs within 75 miles - NO MAXIMUM LIMIT  
    // 6 MINIMUM pairs per lane, but use ALL available pairs found (could be 8, 12, 15+)
    const MIN_UNIQUE_KMAS_PER_SIDE = 6;  // Minimum required, but NO maximum
    ...
    // Find ALL cities within 75 miles - get maximum diversity like your manual process  
    // Business requirement: Never exceed 75 miles for freight intelligence
    const pickupAlternatives = await findCitiesNearLocation(baseOrigin, TARGET_UNIQUE_KMAS_PER_SIDE * 3, 75, usedCities, equipment, false);
    const deliveryAlternatives = await findCitiesNearLocation(baseDest, TARGET_UNIQUE_KMAS_PER_SIDE * 3, 75, usedCities, equipment, false);
    ...
    // Select ALL pickups with unique KMAs (no artificial limit like your manual process)
    for (const city of pickupsSorted) {
      if (!pickupKmasSeen.has(city.kma_code)) {
        ...
        // NO BREAK - collect ALL unique KMAs within 75 miles for maximum diversity
      }
    }
    
    // Select ALL deliveries with unique KMAs (no artificial limit like your manual process)
