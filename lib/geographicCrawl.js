...
export async function generateGeographicCrawlPairs({ origin, destination, equipment, preferFillTo10, usedCities = new Set() }) {
  try {
    ...
    const generationSummary = `Generated ${finalPairs.length}/${target} pairs with ${uniquePickupKmas} pickup KMAs and ${uniqueDeliveryKmas} delivery KMAs`;
    
    if (finalPairs.length >= MIN_REQUIRED_PAIRS) {
      if (finalPairs.length >= target) {
        console.log(`✅ OPTIMAL SUCCESS: ${generationSummary}`);
      } else {
        console.log(`✅ SUFFICIENT SUCCESS: ${generationSummary} (exceeds minimum ${MIN_REQUIRED_PAIRS})`);
      }
    } else {
      console.log(`❌ INSUFFICIENT PAIRS: ${generationSummary} (below minimum ${MIN_REQUIRED_PAIRS})`);
    ...
    return {
      ...
      kmaAnalysis: {
        ...
        uniquePickupKmas,
        uniqueDeliveryKmas,
        success: finalPairs.length >= MIN_REQUIRED_PAIRS,
        fallbackUsed: finalPairs.length > filteredPairs.length
      }
    };
    ...
    console.error(`   Target: ${preferFillTo10 ? '10' : MIN_REQUIRED_PAIRS} pairs`);
    console.error(`   Error: ${error.message}`);
    console.error('   Stack:', error.stack);
    
    // Don't swallow errors - propagate them for proper handling
    throw error;
  }
}
