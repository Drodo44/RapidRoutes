...
export function trackIntelligence(type, details) {
  ...
  // Skip DB writes in offline diagnostics
  if (process.env.RR_DIAGNOSTICS_OFFLINE === '1') {
    return;
  }

  // Save to database for long-term learning
  supabaseAdmin.from('intelligence_log').insert({
    type,
    details,
...
export async function logOperation(type, details) {
  if (process.env.RR_DIAGNOSTICS_OFFLINE === '1') {
    console.log(`[OFFLINE] logOperation(${type}) skipped.`);
    return;
  }
  try {
...
export async function monitorDataQuality() {
  ...
  try {
    ...
    const { data: kmaCount } = await supabaseAdmin
      .from('cities')
      .select('*', { count: 'exact' })
    ...
    const { data: hereCount } = await supabaseAdmin
      .from('cities')
      .select('*', { count: 'exact' })
