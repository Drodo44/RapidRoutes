...
export function verifyBusinessRequirements(csvString, lanes) {
  ...
  const requirements = {
    ...
    errors: [],
    requirements: {
      minimumRowsPerLane: 12, // 6 pairs × 2 contact methods
      expectedLanes: lanes.length,
      expectedTotalRows: lanes.length * 12,
  ...
  try {
    ...
    // Validate minimum rows requirement
    if (requirements.actual.totalRows < requirements.requirements.expectedTotalRows) {
      requirements.errors.push({
        ...
        message: `Expected minimum ${requirements.requirements.expectedTotalRows} rows, found ${requirements.actual.totalRows}`,
        expected: requirements.requirements.expectedTotalRows,
    ...
    monitor.log(requirements.valid ? 'info' : 'error',
      ...
      { 
        ...
        expectedRows: requirements.requirements.expectedTotalRows,
