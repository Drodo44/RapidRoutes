...
export class FreightIntelligence {
    ...
    generateUniquePairs(pickupCities, deliveryCities) {
        ...
        for (const pickup of sortedPickups) {
            ...
            // Try to find up to 11 pairs but guarantee at least 6
            if (pairs.length >= 11 || (pairs.length >= 6 && !this.hasViablePairs(sortedPickups, sortedDeliveries, usedPickupKmas, usedDeliveryKmas))) break;
        }

        // Validation of minimum requirement
        if (pairs.length < 6) {
            throw new Error('Could not generate minimum required 6 pairs with KMA diversity');
        }
