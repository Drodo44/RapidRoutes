    ...
    try {
      ...
      try {
        const diverseResult = await generateDiverseCrawlPairs({
          ...
          destination,
          equipment,
          minimumPairs: 6, // 1 original + minimum 5 intelligent, no maximum
          usedCities: new Set()
        });
...
export async function generateDiversePairs(origin, destination, equipmentCode) {
  console.log('🔍 DEBUG: Input to generateDiversePairs:', {
    origin,
    destination,
    equipmentCode
  });

  // Removed call to generateDiverseCrawlPairs
  console.log('⚠️ DEBUG: Skipping diverse crawl, directly triggering HERE.com fallback');
  const fallbackPairs = await generateGeographicCrawlPairs(origin, destination, equipmentCode);
  console.log('🔍 DEBUG: Output from generateGeographicCrawlPairs:', fallbackPairs);
