...
export class FreightIntelligence {
  ...
  async findDiverseKMACities(baseCity, count = 10) {
    ...
    // For FL lanes, add major FL cities to support long deadheading patterns
    let allCities = cities;
    if (baseCity.state_or_province === 'FL' || baseCity.state === 'FL') {
      const flCities = await this.addMajorFloridaCities('FL', 'FL');
      if (flCities.length > 0) {
        console.log(`[FreightIntelligence] Adding ${flCities.length} major FL cities for FL lane`);
        // Calculate distances for FL cities and merge
        const flCitiesWithDistance = flCities.map(city => ({
          ...city,
        ...
        const cityKeys = new Set(cities.map(c => `${c.city}|${c.state_or_province || c.state}`));
        const uniqueFLCities = flCitiesWithDistance.filter(
          c => !cityKeys.has(`${c.city}|${c.state_or_province || c.state}`)
        );
        allCities = [...cities, ...uniqueFLCities];
        console.log(`[FreightIntelligence] Added ${uniqueFLCities.length} unique major FL cities`);
      }
    }
    
    // Log diversity metrics
    await this.trackIntelligence('city_diversity', {
      baseCity: `${baseCity.city}, ${baseCity.state_or_province || baseCity.state}`,
    ...
    const selected = allCities.slice(0, count);
  ...
  /**
   * Add major Florida cities for FL lanes (drivers commonly deadhead from all over FL)
   * User-provided comprehensive list to support 300-500 mile deadheading patterns
   */
  async addMajorFloridaCities(originState, destinationState) {
    // Only apply this for FL lanes
    ...
    try {
      const { data: flCities, error } = await this.supabase
        .from('cities')
        .select('*')
      ...
      if (error) {
        console.warn('[FreightIntelligence] Could not fetch major FL cities:', error);
        return [];
      }

      console.log(`[FreightIntelligence] Added ${flCities?.length || 0} major FL cities for broader coverage`);
      return flCities || [];
    } catch (error) {
      console.error('[FreightIntelligence] Error fetching major FL cities:', error);
      return [];
    }
  }
  ...
  async generateDiversePairs({ origin, destination, equipment }) {
    ...
    try {
      ...
      // CRITICAL FIX: Enrich origin and destination with coordinates BEFORE calling crawl functions
      console.log('� Enriching origin and destination cities with coordinates and KMAs...');
      let enrichedOrigin = origin;
      let enrichedDestination = destination;
      
      if (!origin.latitude || !origin.longitude) {
        enrichedOrigin = await this.getCityWithKMA(origin);
        console.log(`Enriched origin: ${enrichedOrigin.city}, ${enrichedOrigin.state}`);
      }
      
      if (!destination.latitude || !destination.longitude) {
        enrichedDestination = await this.getCityWithKMA(destination);
        console.log(`Enriched destination: ${enrichedDestination.city}, ${enrichedDestination.state}`);
      }
      
      console.log('�🛠️ Debug: Inputs to generateDiversePairs:', { origin: enrichedOrigin, destination: enrichedDestination, equipment });
